...
Testing is done against the versions of Rails listed in the `Appraisals` file, using Docker.

```
  make build
```

To test all rails versions in `Appraisals`,

``` 
  make test-all
```

To test a single rails version (for example, rails 5)
```
   make test-rails5
```

The test scripts are in bash, which is what the authors of the gem provided. They create a dummy rails app configured for sqlite,
install the health_check gem, and then run up tests against the server.
